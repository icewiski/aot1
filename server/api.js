...
const typeValidator = validate({
  params: { type: joi.string().valid(['castle', 'city', 'town', 'ruin', 'landmark', 'region','character','stark','Lannister']).required() }
})
...
router.get('/characters/:name',  async ctx => {
  console.log('api1',ctx)
  const name = ctx.params.name
  const result = await database.getcharacters(name)
  console.log('api2')
  ctx.body = result || ctx.throw(404)
})
